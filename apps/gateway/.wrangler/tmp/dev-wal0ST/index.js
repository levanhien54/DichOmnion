...
// .wrangler/tmp/bundle-FS5zHQ/checked-fetch.js
...
// .wrangler/tmp/bundle-FS5zHQ/strip-cf-connecting-ip-header.js
...
// ../../packages/crypto-utils/dist/ecdsa.js
var require_ecdsa = __commonJS({
  "../../packages/crypto-utils/dist/ecdsa.js"(exports) {
    "use strict";
    init_checked_fetch();
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
    ...
    __name(generateECDSAKeyPair, "generateECDSAKeyPair");
    async function signPayload(payload, privateKeyJwk) {
      const cryptoAPI = globalThis.crypto;
      const privateKey = await cryptoAPI.subtle.importKey("jwk", privateKeyJwk, { name: "ECDSA", namedCurve: "P-256" }, true, ["sign"]);
      const encoder2 = new TextEncoder();
      const data = encoder2.encode(payload);
      const signatureBuffer = await cryptoAPI.subtle.sign({ name: "ECDSA", hash: { name: "SHA-256" } }, privateKey, data);
      return bufferToBase64(signatureBuffer);
    }
    __name(signPayload, "signPayload");
    async function verifySignature2(payload, signatureBase64, publicKeyJwk) {
      const cryptoAPI = globalThis.crypto;
      const publicKey = await cryptoAPI.subtle.importKey("jwk", publicKeyJwk, { name: "ECDSA", namedCurve: "P-256" }, true, ["verify"]);
      const encoder2 = new TextEncoder();
      const data = encoder2.encode(payload);
      const signatureBytes = base64ToBuffer(signatureBase64);
      return await cryptoAPI.subtle.verify({ name: "ECDSA", hash: { name: "SHA-256" } }, publicKey, signatureBytes, data);
    }
...
var require_dist = __commonJS({
  "../../packages/crypto-utils/dist/index.js"(exports) {
    "use strict";
    init_checked_fetch();
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
...
var require_dist2 = __commonJS({
  "../../packages/shared-types/dist/index.js"(exports) {
    "use strict";
    init_checked_fetch();
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.deterministicStringify = deterministicStringify2;
    function deterministicStringify2(obj) {
      if (obj === null)
        return "null";
      if (typeof obj !== "object")
        return JSON.stringify(obj);
      if (Array.isArray(obj)) {
        return "[" + obj.map(deterministicStringify2).join(",") + "]";
      }
      const keys = Object.keys(obj).sort();
      const props = keys.map((key) => `${JSON.stringify(key)}:${deterministicStringify2(obj[key])}`);
      return "{" + props.join(",") + "}";
    }
    __name(deterministicStringify2, "deterministicStringify");
  }
});

// .wrangler/tmp/bundle-FS5zHQ/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-FS5zHQ/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
// src/index.ts
var import_crypto_utils = __toESM(require_dist());
var import_shared_types = __toESM(require_dist2());
var app = new Hono2();
var KV_STORE = /* @__PURE__ */ new Map();
...
app.post("/api/auth/register", async (c) => {
  try {
    ...
    KV_STORE.set(deviceId, body.publicKeyJwk);
...
app.post("/api/jobs/create", async (c) => {
  ...
  const publicKeyJwk = KV_STORE.get(deviceId);
  if (!publicKeyJwk) {
  ...
  const payloadStr = (0, import_shared_types.deterministicStringify)(payloadObj);
  const isValid = await (0, import_crypto_utils.verifySignature)(payloadStr, signature, publicKeyJwk);
  if (!isValid) {
    return c.json({ error: "Tampering Detected. Signature Invalid." }, 403);
  }
  const now = Date.now();
  if (now - payloadObj.timestamp > 3e4) {
    return c.json({ error: "Request Expired. Replay attack prevented." }, 403);
  }
  const startTime = Date.now();
  const secret = new TextEncoder().encode("OMNIVOICE_ZERO_TRUST_INTERNAL_V1");
  const gatewayJwt = await new SignJWT({ authorized: true, role: "gateway" }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("1m").sign(secret);
  try {
    const gpuWorkerUrl = "http://127.0.0.1:8000/api/worker/process";
    const workerPayload = {
      job_id: payloadObj.jobId,
      audio_url: payloadObj.videoAudioUrl,
      target_language: payloadObj.config.targetLanguage,
      translation_style: payloadObj.config.translationStyle
    };
    const workerRes = await fetch(gpuWorkerUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${gatewayJwt}`
        // Kẹp Token vào Header
      },
      body: JSON.stringify(workerPayload)
    });
    if (!workerRes.ok) {
      if (workerRes.status === 403) {
        return c.json({ error: "Worker Rejected Gateway (Signature Mismatch)" }, 403);
      }
      return c.json({ error: "GPU Worker failed to process job" }, 500);
    }
    const workerData = await workerRes.json();
    const elapsed = Date.now() - startTime;
    if (elapsed < 2e3) {
      return c.json({
        error: `FRAUD DETECTED: Worker tr\u1EA3 k\u1EBFt qu\u1EA3 \u1EA3o qu\xE1 nhanh (${elapsed}ms). \u0110\xE3 g\u1EEDi l\u1EC7nh Terminate GPU!`,
        action: "KILL_WORKER"
      }, 403);
    }
    return c.json({
      message: "Job Processed by GPU Worker!",
      gatewayJobId: payloadObj.jobId,
      timeElapsedMs: elapsed,
      workerResult: workerData
    }, 200);
  } catch (error) {
    return c.json({ error: "Failed to contact GPU Worker. Is it running?" }, 500);
  }
});
var src_default = app;
